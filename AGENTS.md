...
**Backend (backend/):**
- Import data: `uv run python -m app.utils.import_data ../2025-Report-Card-Public-Data-Set.xlsx`
...
- NEVER add any ads such as "Generated with [Codex](https://openai/codex)" in your commit messaages
...
- Playwright CLI default reporter can block waiting for the HTML UI when tests fail. Always pass `--reporter=list` (e.g. `npm run test:e2e -- --reporter=list`) so results stream in the terminal without hanging new Codex sessions.
